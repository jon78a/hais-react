...
import { SearchBarUx } from "../major.ux/SearchBarUx";
import type { SearchMode } from "../../../../schema/types/AdminMajor";
        ...
        <TabPanel value={'UNIV'} sx={{p: 0, mt: 4}}>
          <div className="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-1">
            <Autocomplete
              ...
              onChange={(e, newValue) => {
                if (newValue)
                  return ux.inputKeyword(newValue.name, "univ");
                return ux.inputKeyword("", "univ");
              }}
              renderInput={(params) => <TextField {...params}
                label="대학명 입력"
                onChange={(e) => { ux.inputKeyword(e.target.value, "univ") }}
                size={"small"}
              />}
            />
            {
              isMatchUniv && (
                <TextField id="outlined-basic" label="학과명 입력" variant="outlined"
                  ...
                  fullNameList.map((value) => (
                    <ListItem sx={{p: 0}}>
                      <ListItemButton onClick={() => {
                          ...
                          py: 0,
                          bgcolor: value.id === selectedMajorId ? "primary.light": "background.paper",
                        }}
                      >
                        <ListItemText id={`major-text-${value.id}`} primary={`${value.univ} ${value.name}`} secondary={`${value.department}`}
                          primaryTypographyProps={{
                            sx: {
                              color: value.id === selectedMajorId ? "primary.main" : undefined,
                            }
                          }}
                        />
                      </ListItemButton>
