...
import { selectedMajorIdState, subjectDataListState } from "../../../../schema/states/AdminMajor";
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="center">교과(군)</TableCell>
            <TableCell />
          </TableRow>
        </TableHead>
        <TableBody>
                ...
                <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
                  ...
                  <TableCell>
                    <IconButton
                      ...
                      size="small"
                      onClick={() => {
                        if (clickedRowCode === row.code) setClickedRowCode(null);
                        else setClickedRowCode(row.code);
                      }}
                    >
                      {clickedRowCode === row.code ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                    </IconButton>
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell style={{ paddingBottom: 0, paddingTop: 0, width: "100%" }} colSpan={6}>
                    <Collapse in={clickedRowCode === row.code} timeout="auto" unmountOnExit sx={{py: 4, px: 1}}>
                      <Stack spacing={2}>
                        <Stack spacing={1}>
                        ...
                        <Stack spacing={1}>
                          ...
                          <Divider/>
                          <Typography variant={"body2"}>{row.suneungInfo}</Typography>
                        </Stack>
                        <Stack spacing={1}>
                          <Typography>기타정보</Typography>
                          <Divider/>
                          <Typography variant={"body2"}>{row.etcInfo}</Typography>
                        </Stack>
                      </Stack>
                    </Collapse>
                  </TableCell>
